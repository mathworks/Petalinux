# Petalinux Project Creation and Linux Image Build for AMD Zynq UltraScale+ RFSoC DFE ZCU670 Evaluation Kit

This Petalinux repo is the starting point to build the Linux image for AMD Zynq UltraScale+ RFSoC DFE ZCU670 Evaluation Kit compatible to MathWorks applications. The Linux image build process also includes the use of meta-mathworks layer for the Linux image to work for MathWorks applications. The meta-mathworks layer is compatible to Petalinux version 2023.2.
...
1. Download the [Petalinux installer v2023.2](https://www.xilinx.com/member/forms/download/xef.html?filename=petalinux-v2023.2-10121855-installer.run)
2. Run the below commands to install the Petalinux tool from the path where you downloaded the petalinux installer.

	```
	chmod 755 ./petalinux-v2023.2-05012318-installer.run
	./petalinux-v2023.2-05012318-installer.run
	```
...
2. Checkout the latest branch after navigating to Petalinux directory.    
	...
	```
Navigate to *mw_ZCU670* directory. You can use `mw_build_ZCU670.sh` script present in this folder to add the meta-mathworks layer to the Petalinux project and get the Linux image in a zip file directly in your working directory. Note that the script is run from its path only using below command.

	chmod +x mw_ZCU670
	cd mw_ZCU670
	chmod 777 *.sh
	./mw_build_ZCU670.sh <petalinux_installed_directory> <mw_ZCU670_directory>

 If you wish to build the Linux image manually, follow the steps mentioned in this section.

3. Add the `meta-mathworks` layer to the ZCU670 Petalinux project using below command. Run this command from the *mw_ZCU670* directory.			
...
With this you are pointing the Petalinux project to use meta-mathworks layer in Linux image build process and this completes the Petalinux project setup. You can now navigate to the path *Petalinux/mw_ZCU670*

Note: This repo is already setup with necessary configurations settings to build the bootable Linux image for ZCU670. If you want to include any specific kernel drivers in your Linux image, you can refer to the **Configure the kernel** section in this file on different ways to configure the kernel.
...
This section explains how to build the MathWorks compatible Linux image for ZCU670 using from the xsa 

### Complete the Petalinux project setup with XSA


1. Create the Petalinux project using the ZCU670 by running the below command. 

	```
	petalinux-create -t project --template zynq --name xilinx-ZCU670-v2023.2
	```

2. Add the `meta-mathworks` layer to the ZCU670 Petalinux project using below command. Run this command from the *mw_ZCU670* directory.		
...
1. cd to the Petalinux project(*xilinx-ZCU670-v2023.2*)
	```
	petalinux-config --get-hw-description=<path-to-mw_ZCU670/ZCU670.xsa>
	```
...
1. Open the `user-rootfsconfig` file present in the path *xilinx-ZCU670-v2023.2/project-spec/meta-user/conf/*.
2. Copy the below packages to include in the Linux image and save the file.
	...
	CONFIG_rfdc-selftest
	CONFIG_mw-rf-init
	CONFIG_rftool-ZCU670
	```
3. Open the `rootfs_config` file present in the path *xilinx-ZCU670-v2023.2/project-spec/configs/rootfs_config*.
4. Copy the below packages to include in the Linux image and save the file.
	...
	CONFIG_mw-rf-init=y
	CONFIG_rftool-ZCU670=y
	```	
	
5. Similarly, you can include your custom packages into the root file system.
6. Navigate to Petalinux project path (*xilinx-ZCU670-v2023.2*).
...
Default kernel settings are used to build the Linux image for ZCU670. If you wish to configure the kernel, you can follow below steps.
...
This completes the Linux image build for ZCU670.Once the build is successful, you can see the Linux binaries present under the path *images/Linux*. 
...
NOTE: If we use the script mw_build_ZCU670.sh then Image Packaging step will be automatically handled and it will generate a xilinx-ZCU670-2023.2_sdcard_zynq_mpsoc_ec_YYYY-MM-DD.zip in <proj-dir/images> directory
