# Petalinux Project Creation and Linux Image Build for AMD Zynq™ UltraScale+™ RFSoC ZCU208 Evaluation Kit
 

This Petalinux repo is the starting point to build the Linux image for AMD Zynq™ UltraScale+™ RFSoC ZCU208 Evaluation Kit
...
2. Checkout the latest branch after navigating to Petalinux directory.    
	...
	```
Navigate to *mw_zcu208* directory. You can use `mw_build_zcu208.sh` script present in this folder to add the meta-mathworks layer to the Petalinux project and get the Linux image in a zip file directly in your working directory. Note that the script is run from its path only using below command.

	chmod +x mw_zcu208
	cd mw_zcu208
	chmod 777 *.sh
	./mw_build_zcu208.sh <petalinux_installed_directory> <mw_zcu208_directory>

 If you wish to build the Linux image manually, follow the steps mentioned in this section.

3. Add the `meta-mathworks` layer to the zcu208 Petalinux project using below command. Run this command from the *mw_zcu208* directory.			
...
With this you are pointing the Petalinux project to use meta-mathworks layer in Linux image build process and this completes the Petalinux project setup. You can now navigate to the path *Petalinux/mw_zcu208*

Note: This repo is already setup with necessary configurations settings to build the bootable Linux image for zcu208. If you want to include any specific kernel drivers in your Linux image, you can refer to the **Configure the kernel** section in this file on different ways to configure the kernel.
...
This section explains how to build the MathWorks compatible Linux image for zcu208 using from the bsp 

### Complete the Petalinux project setup with BSP

1. Download the [ AMD Zynq® UltraScale+™ MPSoC zcu208 Evaluation Kit](https://www.xilinx.com/member/forms/download/xef.html?filename=xilinx-zcu208-v2023.1-05080224.bsp).

2. Create the Petalinux project using the zcu208 BSP by running the below command.

	```
	petalinux-create -t project -s <path_to_downloaded_bsp>
	```
	After this a new project with default name *xilinx-zcu208-v2023.1* is created. You can also create the project with specific name in specific directory by providing extra arguments to the above command. For more information, see [Creating a Project from a BSP](https://docs.amd.com/r/2023.1-English/ug1144-petalinux-tools-reference-guide/Creating-a-Project-from-a-BSP).	
...
1.  Change directory to the Petalinux project(*xilinx-zcu208-v2023.1*)
...
1. Open the `user-rootfsconfig` file present in the path *xilinx-zcu208-v2023.1/project-spec/meta-user/conf/*.
2. Copy the below packages to include in the Linux image and save the file.
	...
	CONFIG_rfdc-selftest
	CONFIG_mw-rf-init
	CONFIG_rftool-zcu208
	```
3. Open the `rootfs_config` file present in the path *xilinx-zcu208-v2023.1/project-spec/configs/rootfs_config*.
...
6. Navigate to Petalinux project path (*xilinx-zcu208-v2023.1*).
...
Default kernel settings are used to build the Linux image for zcu208. If you wish to configure the kernel, you can follow below steps.
...
This completes the Linux image build for zcu208.Once the build is successful, you can see the Linux binaries present under the path *images/Linux*. 
...
NOTE: If we use the script mw_build_zcu208.sh then Image Packaging step will be automatically handled and it will generate a xilinx-zcu208-2023.1_sdcard_zynq_mpsoc_ec_YYYY-MM-DD.zip in <proj-dir/images> directory
