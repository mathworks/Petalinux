# Petalinux Project Creation and Linux Image Build for AMD Zynq® UltraScale+™ MPSoC ZCU102 Evaluation Kit

This Petalinux repo is the starting point to build the Linux image for AMD Zynq® UltraScale+™ MPSoC ZCU102 Evaluation Kit compatible to MathWorks applications. The Linux image build process also includes the use of meta-mathworks layer for the Linux image to work for MathWorks applications. The meta-mathworks layer is compatible to Petalinux version 2023.1.
...
2. Checkout the latest branch after navigating to Petalinux directory.    
	...
	```
Navigate to *mw_zcu102* directory. You can use `mw_build_zcu102.sh` script present in this folder to add the meta-mathworks layer to the Petalinux project and get the Linux image in a zip file directly in your working directory. Note that the script is run from its path only using below command.

	chmod +x mw_zcu102
	cd mw_zcu102
	chmod 777 *.sh
	./mw_build_zcu102.sh <petalinux_installed_directory> <mw_zcu102_directory>

 If you wish to build the Linux image manually, follow the steps mentioned in this section.

3. Add the `meta-mathworks` layer to the zcu102 Petalinux project using below command. Run this command from the *mw_zcu102* directory.			
...
With this you are pointing the Petalinux project to use meta-mathworks layer in Linux image build process and this completes the Petalinux project setup. You can now navigate to the path *Petalinux/mw_zcu102*

Note: This repo is already setup with necessary configurations settings to build the bootable Linux image for zcu102. If you want to include any specific kernel drivers in your Linux image, you can refer to the **Configure the kernel** section in this file on different ways to configure the kernel.
...
This section explains how to build the MathWorks compatible Linux image for ZCU102 using from the bsp 

### Complete the Petalinux project setup with BSP

1. Download the [ AMD Zynq® UltraScale+™ MPSoC ZCU102 Evaluation Kit](https://www.xilinx.com/member/forms/download/xef.html?filename=xilinx-zcu102-v2023.1-05080224.bsp).

2. Create the Petalinux project using the ZCU102 BSP by running the below command.

	```
	petalinux-create -t project -s <path_to_downloaded_bsp>
	```
	After this a new project with default name *xilinx-ZCU102-v2023.1* is created. You can also create the project with specific name in specific directory by providing extra arguments to the above command. For more information, see [Creating a Project from a BSP](https://docs.amd.com/r/2023.1-English/ug1144-petalinux-tools-reference-guide/Creating-a-Project-from-a-BSP).	
...
1.  Change directory to the Petalinux project(*xilinx-ZCU102-v2023.1*)
...
1. Open the `user-rootfsconfig` file present in the path *xilinx-zcu102-v2023.1/project-spec/meta-user/conf/*.
2. Copy the below packages to include in the Linux image and save the file.
	...
	```
3. Open the `rootfs_config` file present in the path *xilinx-zcu102-v2023.1/project-spec/configs/rootfs_config*.
...
6. Navigate to Petalinux project path (*xilinx-zcu102-v2023.1*).
...
Default kernel settings are used to build the Linux image for zcu102. If you wish to configure the kernel, you can follow below steps.
...
This completes the Linux image build for zcu102.Once the build is successful, you can see the Linux binaries present under the path *images/Linux*. 
...
copy the below files from /images/linux/ into the SD card
	...
	system.bit
	system.dtb  
	mw_zcu102/mw_utils/fw_env.config
	mw_zcu102/mw_utils/init.sh
	mw_zcu102/mw_utils/interfaces
	mw_zcu102/mw_utils/sshd_config

NOTE: If we use the script mw_build_zcu102.sh then Image Packaging step will be automatically handled and it will generate a xilinx-zcu102-2023.1_sdcard_zynq_mpsoc_ec_YYYY-MM-DD.zip in <proj-dir/images> directory
