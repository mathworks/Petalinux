# Petalinux Project Creation and Linux Image Build for Picozed with Imageon Carrier Card

This Petalinux repo is the starting point to build the Linux image for Picozed with Imageon Carrier Card compatible to MathWorks applications. The Linux image build process also includes the use of meta-mathworks layer for the Linux image to work for MathWorks applications. The meta-mathworks layer is compatible to Petalinux version 2023.1.
...
2. Checkout the latest branch after navigating to Petalinux directory.    
	...
	```
Navigate to *mw_picozed* directory. You can use `mw_build_picozed.sh` script present in this folder to add the meta-mathworks layer to the Petalinux project and get the Linux image in a zip file directly in your working directory. Note that the script is run from its path only using below command.

	chmod +x mw_picozed
	cd mw_picozed
	chmod 777 *.sh
	./mw_build_picozed.sh <petalinux_installed_directory> <mw_picozed_directory>

 If you wish to build the Linux image manually, follow the steps mentioned in this section.

3. Add the `meta-mathworks` layer to the picozed Petalinux project using below command. Run this command from the *mw_picozed* directory.			
...
With this you are pointing the Petalinux project to use meta-mathworks layer in Linux image build process and this completes the Petalinux project setup. You can now navigate to the path *Petalinux/mw_picozed*

Note: This repo is already setup with necessary configurations settings to build the bootable Linux image for picozed. If you want to include any specific kernel drivers in your Linux image, you can refer to the **Configure the kernel** section in this file on different ways to configure the kernel.
...
This section explains how to build the MathWorks compatible Linux image for picozed using from the xsa 

### Complete the Petalinux project setup with XSA


1. Create the Petalinux project using the picozed by running the below command. 

	```
	petalinux-create -t project --template zynq --name xilinx-picozed-v2023.1
	```

2. Add the `meta-mathworks` layer to the picozed Petalinux project using below command. Run this command from the *mw_picozed* directory.
...
1. cd to the Petalinux project(*xilinx-picozed-v2023.1*)
	```
	petalinux-config --get-hw-description=<path-to-mw_picozed/picozed.xsa>
	```
...
1. Open the `user-rootfsconfig` file present in the path *xilinx-picozed-v2023.1/project-spec/meta-user/conf/*.
2. Copy the below packages to include in the Linux image and save the file.
	...
	```
3. Open the `rootfs_config` file present in the path *xilinx-picozed-v2023.1/project-spec/configs/rootfs_config*.
...
6. Navigate to Petalinux project path (*xilinx-picozed-v2023.1*).
...
Default kernel settings are used to build the Linux image for picozed. If you wish to configure the kernel, you can follow below steps.
...
This completes the Linux image build for picozed.Once the build is successful, you can see the Linux binaries present under the path *images/Linux*. 
...
copy the below files from /images/linux/ into the SD card
	...
	system.bit
	system.dtb  
	mw_picozed/mw_utils/fw_env.config
	mw_picozed/mw_utils/init.sh
	mw_picozed/mw_utils/interfaces
	mw_picozed/mw_utils/sshd_config

NOTE: If we use the script mw_build_picozed.sh then Image Packaging step will be automatically handled and it will generate a xilinx-picozed-2023.1_sdcard_zynq7000ec_YYYY-MM-DD.zip in <proj-dir/images> directory
