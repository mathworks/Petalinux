...
4. Copy the `mw_kr260.patch` file to *meta-mathworks* folder using below command.		

	```
	cp -f mw_kr260.patch project-spec/meta-mathworks/
	```

5. Navigate to *meta-mathworks* folder and apply the `mw_kr260.patch` by running the below command.	

	```
	git apply mw_kr260.patch
	```

6. Point the Petalinux project to the `mata-mathworks` layer. You can do this by using Petalinux config GUI. Run the below command to launch the Petalinux config GUI.
...
1. Download the [AMD Kria KR260 Robotics Starter kit Petalinux BSP](https://www.xilinx.com/member/forms/download/xef.html?filename=xilinx-kr260-starterkit-v2023.1-05080224.bsp).

2. Create the Petalinux project using the KR260 BSP by running the below command. 
...
1. cd to the Petalinux project(*xilinx-kr260-starterkit-2023.1*)
...
This completes the Linux image build for KR260.Once the build is successful, you can see the Linux binaries present under the path *images/Linux*. 
...
The `init.sh` script is executed immediately after the board is booted up. This script contains the utility commands to make your `BOOT.BIN` as default binary to use for bootup on every POR of the target and also to copy the raw booting environment from the QSPI memory to the SD card FAT partition. You can also customize this script as per your requirement by including the commands which you want to be run immediately after succeful booting up of the board.
...
Note: Also, everytime you power cycle the board, MathWorks provided `BOOT.BIN` file will be used for initial boot. However, you can change the `.BIN` file or point to Xilinx provided `.BIN` files by using the above command.
